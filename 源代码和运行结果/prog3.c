...
sendrtpkt(srcid, destid, mincosts) int srcid;
int destid;
int mincosts[];
{
    struct rtpkt packet;
    ...
    for (i = 0; i < 4; i++) packet.mincost[i] = mincosts[i];
    tolayer2(packet);
    printf(
        "At time t=%.3f, node %d sends packet to node %d with: (%d  %d  %d  "
        "%d)\n",
        clocktime, packet.sourceid, packet.destid, packet.mincost[0],
